...
仓库创建完成即可上传应用到平台进一步测试和部署应用到 Kubernetes 运行环境中，可参考 [开发者 - 上传应用](../ISV-quick-start/#第二步：上传应用)。部署 Helm Chart 类型的应用（如本篇提到的 nginx）到 Kubernetes 运行环境，参见 [部署 Nginx 到 Kubernetes](../kubernetes-quick-start)。

...
以下安装方式仅供参考，请以 [Helm 官方安装文档](https://github.com/helm/helm/blob/master/docs/install.md#installing-the-helm-client) 和 [Tiller 官方安装文档](https://github.com/helm/helm/blob/master/docs/install.md#installing-tiller) 为标准。
...
执行 helm init，该命令会在当前目录下创建 helm 文件夹即 `~/.helm`，并且通过 Kubernetes Deployment 部署 Tiller，检查 Tiller 是否成功安装：
...
2、查看 Pilot 服务，Pilot 用于接受来自集群服务的指令和信息的组件，如创建集群等，并可以传递指令给 Frontgate，它还接收来自 Frontgate 上传上来的信息。以下可以看到两个端口，依次是 https 和 http 协议的端口，Pilot 服务 http 协议的 9114 端口对应的端口，例如以下返回的是 30119，因此 Pilot 服务的端口需要暴露给外部访问（可能需要端口转发和防火墙放行该端口）：
...
3、修改 `values.yaml` 文件中的配置 `global_config` 下的 `pilot_ip` 和 `pilot_port` 为您实际的公网 IP (EIP) 和 Pilot 服务的端口号 (NodePort)。由于 Pilot 是需要公网访问，所以需要保证在外网能访问上述的 Pilot 服务。

> 若公网 IP 有防火墙，请在防火墙添加规则放行对应的端口，外部才能够访问。

```bash
global_config:
  pilot_ip: 139.198.121.143
  pilot_port: 30119
  ···
```

4、Helm 使用 upgrade 命令来更新已安装的 release，由于上一步修改了配置文件，因此执行如下命令更新 release：
...
1、查看 Dashboard 服务，若需要通过外网访问，可能需要进行端口转发和防火墙放行等操作。
...
2、您可以通过浏览器，使用集群中任一节点的 IP 地址和端口号即 `<NodeIP>:<NodePort>` 可在集群内部访问 Dashboard，如 `http://192.168.100.10:31879`。也可以通过公网 IP 并将端口转发后访问控制台，如：`http://139.198.121.143:31879`，即可进入 OpenPitrix 主页面。
...
执行卸载命令将从环境中删除 OpenPitrix 所有服务，卸载操作不可逆，请谨慎操作。
